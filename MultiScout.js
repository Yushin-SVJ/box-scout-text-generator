...
代わりに、「これらの企業に最もフィットする候補者はどんなタイプか？」を先に決め、そのペルソナに合わせた戦略を選んでください。
...
### 【STEP 1: 各社の分析】
それぞれの単社スカウト本文を読み、各社について以下を整理してください：
- その企業の「最大のウリ（Growth Factor）」
- 会社概要（2〜3文程度の要約）

### 【STEP 2: 2社の共通点・相違点の特定】
2社を横並びで見比べ、以下を特定してください：
- **共通するキャリア軸・魅力**（例：「成長フェーズで裁量を持てる」「営業経験を土台に事業づくりに関われる」）
- **際立つ個性・違い**（例：「A社は安定基盤、B社は挑戦フェーズ」）
この共通点と相違点が、メール全体のストーリーの軸になります。

### 【STEP 3: アプローチ戦略の策定（A/B/C）】
2社の組み合わせと、想定されるターゲットペルソナに基づいて、戦略を選択してください。

**【複数社の場合の戦略選択ルール】**
- 2社とも同じタイプの場合 → そのタイプに対応する戦略を選択
- 2社のタイプが異なる場合 → より「慎重派」寄りの戦略を選択（A > B > C の優先度）
  - 理由：複数社を比較提示する形式は、そもそも「比較検討したい」候補者向けのため
...
- **C：効率インパクト戦略（Efficiency Impact）**
  ...
  - **【重要】Cを選ぶ前に以下の3条件をすべて確認してください**:
    ...
    2. 2社とも「あ、聞いたことある」と言えるレベルの知名度があるか？（リクルート、マイナビ級）
    3. 「資金調達額」「成長率」「IPO準備」だけがCを選ぶ理由になっていないか？
  - **3条件すべてを満たさない場合、AまたはBを選んでください。**

### 【STEP 4: スカウト文面の執筆】
選択した戦略に基づき、2社をまとめて紹介するスカウト文を作成してください。
...
- JSON に必ず以下のフィールドを含めること:
  ...
  - "pattern": 選択した戦略（A / B / C）
  - "pattern_reason": 選択理由を日本語で（例: "2社とも社会課題解決がミッションの中心であるため、共感を生むB戦略を選択"）。
-------------------------------------
【構成に関する詳細ルール】
【全体共通ルール】
